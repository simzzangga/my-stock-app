...
def check_password():
    ...
    st.title("💰 Shim's MSM Portal (FDR 3.11 Optimized)")
...
def run_analysis(ticker, base_date, mode):
    ...
    if df.empty or len(df) < 10: 
        return None, "데이터를 불러올 수 없거나 기간이 짧습니다."

    # 최신 종가 데이터 추출 (iloc 사용으로 안전하게 접근)
    current_price = int(df['종가'].iloc[-1])
    
    if "v3.3" in mode:
        # v3.3: 수급 중심
        spikes = df[df['등락률'] >= 15]
        if spikes.empty: return None, "최근 급등 패턴 없음"
        ...
        return {
            "spike_date": s_date.strftime("%Y-%m-%d"), 
        ...
        if spikes.empty: return None, "v4.0 기준봉 조건 미달"
...
if check_password():
    ...
    with st.container(border=True):
        ...
        analysis_date = c2.date_input("기준 날짜", default_date)
    ...
    # [B] 고속 스캐너 (FDR 최적화)
    st.markdown("### 📡 시장 스캐너")
    if st.button("🚀 전 종목 스캔 (급등 종목 대상)", use_container_width=True):
        with st.status("데이터 분석 중...", expanded=True) as status_box:
            st.write("1단계: 전체 시장 시세 로드...")
            try:
                # FDR 리스팅 정보를 활용하여 필터링
                df_list = fdr.StockListing('KRX')
                # 전일대비 등락률이 높은 후보군 필터링 (FDR 컬럼명 기준)
                candidates = df_list[df_list['ChgPct'] >= 10]['Code'].tolist()
                
                st.write(f"2단계: {len(candidates)}개 후보 종목 정밀 분석...")
                found = []
                for t in candidates:
                    res_scan, status_scan = run_analysis(t, analysis_date, app_mode)
                    if res_scan and ("매수" in status_scan or "추천" in status_scan):
                        found.append({
                            "종목명": ticker_dict.get(t, t),
                            "코드": t,
                            "상태": status_scan,
                            "현재가": f"{res_scan['current_price']:,}원",
                            "목표가": f"{res_scan['target']:,}원"
                        })
                ...
                else: st.write("현재 조건에 부합하는 종목이 없습니다.")
            except Exception as e:
                st.error(f"스캔 도중 오류가 발생했습니다: {e}")
