...
def run_antigravity_analysis(ticker, base_date):
    ...
    # [로직변경] 의미있는 기준봉 탐색 (15% 이상 & 거래량 3배 이상 폭증)
    ...
    after_spike = df.loc[spike_date:].iloc[1:] # 기준봉 이후 데이터
    ...
    if not ma_ok: return None, "이평선 정배열 미달"

    # 신호 판정 (v4.0 단계별 매수 타점)
    status = None
    ...
    reliability = 100
    if vol_ratio > 0.30: reliability -= 20
...
if check_password():
    ...
    if res:
        ...
        st.table(pd.DataFrame({
            "항목": ["최근 기준봉", "현재가", "1차 지지선", "2차 지지선", "3차 지지선", "손절가", "목표가"],
